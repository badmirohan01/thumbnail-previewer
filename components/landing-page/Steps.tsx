...
const stepsArray = [
    ...
    { icon: Layout, step: "Step 2", description: "", image: thumbnail2 },
    { icon: Eye, step: "Step 3", description: "", image: thumbnail3 }
]

export default function Steps() {
    return (
        <AnimationContainer className="md:mx-40 mt-20 relative">
            ...
            <AnimationContainer delay={0.2} className="grid md:grid-cols-3">
                ...
                {stepsArray.map((step, index) => (
                    <div key={index} className="relative group p-8 transition-all duration-300 bg-background/30 hover:bg-white/5 backdrop-blur-sm shadow-lg hover-shadow-xl">
                        <div className="mb-6 inline-flex items-center gap-3 p-4 rounded-2xl border border-primary/10">
                            <div className="p-2 bg-primary/10 rounded-xl">
                                <Pencil className="h-5 w-5 text-primary" />
                            </div>
                            <h2 className="text-2xl font-medium text-primary tracking-primary">
                                {step.step}
                            </h2>
                        </div>

                        <p className="text-sm text-muted-foreground md:mb-8">
                            {step.description}
                        </p>
